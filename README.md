...
2. 把它上传到空网站的根目录；
3. 访问 `你的域名/install.php`，填写网站和管理员信息，点击安装。

安装页面会自动检查 PHP 和目录权限，把账号、内容与备份放到公开目录之外，并在成功
...
- 最低支持 PHP 7.4；正式网站推荐 PHP 8.2 或更高版本；
- PDO SQLite、Fileinfo 和 GD；支持 WebP 时优先使用，不支持时自动改用 JPG 或 PNG；
- Sodium 有则用于增强安装检查，没有也能安装；
- EXIF 可选；
- HTTP 也可以安装和使用；正式网站建议开启 HTTPS。安装器会优先把数据库和备份放在网站根目录之外；上一级不可写时，会自动改用根目录内受 Apache/LiteSpeed 规则保护的私有目录。

PHP 7.4 兼容是为了让现有虚拟主机可以安装，不代表它仍适合长期承载重要业务。
新建正式站点时，建议在主机面板选择 PHP 8.2 或更高版本。
...
正式网址，再选择网站风格。新网站默认使用“清简商务”，同时可以切换到 Studio；以后
